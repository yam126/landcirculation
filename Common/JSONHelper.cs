    ...
    public class JSONHelper
    {
        ...
        public static List<T> JSONToList<T>(string JsonString, out string message)
        {
            ...
            try
            {
                ...
                objs = JsonSerializer.Deserialize<List<T>>(Ru);
            }
            catch (Exception exp)
            {
                message = exp.Message;
        ...
        public static string ListToJSON<T>(List<T> vList, string newLineSymbol, out string message)
        {
            ...
            for (int i = 0; i < vList.Count; i++)
            {
                ...
                {
                    if (i == vList.Count - 1)
                        JsonString += JsonSerializer.Serialize(vList[i]) + newLineSymbol;
                    else
                        JsonString += JsonSerializer.Serialize(vList[i]) + "," + newLineSymbol;
                }
                catch (Exception exp)
        ...
        /// List转换成Json
        /// </summary>
        /// <typeparam name="T">List的泛型类型</typeparam>
        ...
        public static DataSet JsonToDataSet(string json)
        {
            ...
            ArrayList dic = JsonSerializer.Deserialize<ArrayList>(json);
            DataTable dtb;
        ...
        /// <param name="type">字符串的数据类型</param>
